...
(defcustom spaceline-always-show-segments nil
  "when true, show all the segments that would otherwise be hidden in
inactive windows.")
    ...
    (let* (;; Handle the different calling conventions
      ...
      (eval `(defun ,target-func ()
               ...
               (let* ((active-strict (powerline-selected-window-active))
                      (active (or spaceline-always-show-segments (powerline-selected-window-active)))
