...
Used as a predicate for `sort' in `spaceline--init-runtime'."
  (let ((pri-1 (spaceline--priority first-alist))
        (pri-2 (spaceline--priority second-alist))
        (len-1 (spaceline--length first-alist))
        (len-2 (spaceline--length second-alist)))
    (if (equal pri-1 pri-2)
        (> len-1 len-2)
      (< pri-1 pri-2))))
